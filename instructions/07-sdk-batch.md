# 複数のポイント操作をバッチ処理する（Azure Cosmos DB for NoSQL SDK）
...
このラボでは、Azure Cosmos DB for NoSQL SDK を使用して、単一のトランザクション ユニット内で複数の操作をバッチ処理する実践的な経験を得ました。`TransactionalBatch` と `TransactionalBatchResponse` クラスを活用することで、複数のポイント操作をグループ化し、すべての操作がまとめて成功するか失敗するかというアトミック性を確保する方法を学びました。また、Cosmos DB ではトランザクション内のアイテムが同じパーティション キーを共有する必要があるため、バッチ操作におけるパーティション キーの扱いが重要であることも強調しました。
