...
このラボでは、テスト .NET アプリケーションを使用して、デフォルトのインデックス ポリシーを利用した場合と、わずかに調整したインデックス ポリシーを利用した場合に、Azure Cosmos DB SQL API コンテナーに大きな JSON アイテムを挿入する方法を学習します。
...
Azure Cosmos DB は複数の API をサポートするクラウドベースの NoSQL データベース サービスです。Azure Cosmos DB アカウントを初めてプロビジョニングする際には、アカウントでサポートする API を選択します（たとえば **Mongo API** や **NoSQL API**）。Azure Cosmos DB for NoSQL アカウントのプロビジョニングが完了したら、エンドポイントとキーを取得し、Azure SDK for .NET または他の任意の SDK を使用して Azure Cosmos DB for NoSQL アカウントに接続できます。
