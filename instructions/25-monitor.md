...
1. Select the **Extension (1)** icon from the left pane. Enter **C# (2)** in the search bar select the **extension (3)** that shows up, and finally select **Install (4)** on the extension. 
...
1.  Navigate to **C:\AllFiles (1)**, select **dp-420-cosmos-db-dev-main (2)**, and click on **Select Folder (3)**
...
1. This pane contains the connection details and credentials necessary to connect to the account from the SDK. Specifically:
    ...
    - Record the value of the **URI** field. You'll use this **endpoint** value later in this exercise.

    - Record the value of the **PRIMARY KEY** field by selecting the show primary key icon. You'll use this **key** value later in this exercise.
...
1. Right-click on the **25-monitor (1)** folder and then select **Open in Integrated Terminal (2)** to open a new terminal instance.

   ![06](media/25-06-25-l12-1.png)
...
1. In **Visual Studio Code**, in the **Explorer** pane, browse to the **25-monitor (2)** folder.

1. Open the **Program.cs (2)** code file.

   ![06](media/25-06-25-l12-2.png)
...
1. Build and run the project using the [dotnet run][docs.microsoft.com/dotnet/core/tools/dotnet-run] command:
    ...
    > &#128221; You will notice a couple of warnings in yellow since the compiler detects that the script runs many operations synchronously and does not wait for replies of the operations. You can ignore these warnings since that is the expected behaviour to run multiple SQL scripts simultaneously.

    >**Note**: Visual Studio Code might crash after running the above command. You can rerun the command and continue to the next task. If Visual Studio is crashing more than 2 times, uninstall and reinstall Visual Studio Code by following the steps below.

      - From the start menu, search and select **Control Panel**.
      ...
      - Once the download is completed, open the downloaded file and install Visual Studio Code.
...
#### Subtask 1: Azure Monitor Metrics' reports

1. Go back to the open browser window we minimized earlier. If you closed it, open a new one and go to your Azure Cosmos DB account page under [Azure Portal](portal.azure.com).

1. In the Azure Cosmos DB left-hand menu, under **Monitoring (1)**, select **Metrics (2)**. You'll notice that the **Scope** and **Metric Namespace** fields are prepopulated with the correct information. In the following steps, we'll take a look at a few **Metric** options and the *Add filter* and *Apply splitting* features.

   ![06](media/25-06-25-l12-3.png)

1. By default, the *Metrics* section will show us diagnostic information for the last 24 hours. We need to get more granular to look at the metrics during the workload we created in the previous step. On the upper right-hand corner, select the button labeled **Local time: Last 24 hours (Automatic) (1)**. We'll then get a window with multiple radio button time range options.  Choose the radio button labeled **Last 30 minutes (2)** and select the **Apply (3)** button. If needed, you can get much more granular by choosing the *Custom* radio button and picking a start and end date and time. 

   ![06](media/25-06-25-l12-4.png)

1. Now that we have a good time range for our diagnostic charts, let's take a look at some Metrics. We'll start with a common metric. From the **Metric (1)** pulldown, choose **Total Request Units**. By default, this metric will be displayed as the total sum of RUs. Or, you can change the Aggregation pulldown to average or max. Once you check out those two aggregations, set it back to **Sum** for the following steps.

   ![06](media/25-06-25-l12-5.png)

1. This metric gives us a good idea of how many request units have been used in our Azure Cosmos DB Account. However, our current chart might not help us hone in on an issue when we have multiple databases or containers in our account. Let's change that, let's review how our RU consumption was done by the Database. In the menu under the chart title, select **Apply splitting (1)**, under the **Values** pulldown choose **DatabaseName**, and select anywhere on the chart to accept the changes. A **Split by = DatabaseName (2)** button will now be placed right above the chart. 

   ![06](media/25-06-25-l12-6.png)

   ![06](media/25-06-25-l12-7.png)
...
1. Two key filters here can help us troubleshoot different types of issues. Let's add a filter with the Property **StatusCode (1)** (notice that a similar filter with a different type of detail would be **Status**), for *Values* pick **200** and **429**. Change the **Split by (2)** to use **StatusCode**. Notice that there's a huge amount of 429, or rate limiting requests, compared to status 200, successful requests. The 429 exceptions happened because the script was sending thousands of requests per second while we set the provisioned throughput to 400 RU/s. *This large number of 429 exceptions compared to successful requests shouldn't be normal in a production environment. In a production environment, 429 exceptions should happen infrequently in a healthy Azure Cosmos DB account*.  You can also use **StatusCode** or **Status** *Properties* in a similar troubleshooting fashion against **Total Request Units**.

   ![06](media/25-06-25-l12-9.png)

1. Let's keep on looking ot the **Total Request**, but lets change the split to **OperationType**.  This property will help us determine which read or write operations are doing the bulk of the work. Again, this property could also be used similarly against **Total Request Units**
...
1. In the Azure Cosmos DB left-hand menu, under **Monitoring (1)**, select **Insights (2)**. You'll notice that there are multiple tabs from Overview to Management Options. We'll look at a few of these **Insight** charts. The first tab, the Overview tab, provides a summary of the most common charts you might use. For example, charts like total requests, Data and Index usage, 429 exceptions, and Normalized RU consumption.  We saw most of these charts in the previous section.

   ![06](media/25-06-25-l12-11.png)
...
1. On the upper right-hand corner of *each* chart, you'll notice an option to **Open Metric Explorer**. Let's go ahead and select the  **Open Metric Explorer (1)** option for the **Total Requests** chart. You'll notice that when you select this option will take you to the Metric reports we reviewed earlier. The advantage of opening the Metric Explorer is that a good portion of the chart has already been built for us.

   ![06](media/25-06-25-l12-12.png)

1. Let's get back to the Insights page by selecting the **X** on the upper right of the Metric chart.

1. Select the Throughput Tab. These charts are good for pinpointing throughput issues. Pay close attention to the **Normalized RU Consumption (%) By PartitionKeyRangeID** chart, which can be used to detect hot partitions.

   ![06](media/25-06-25-l12-13.png)
