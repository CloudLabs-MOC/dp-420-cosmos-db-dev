# Lab 11d - Azure Cosmos DB SQL API ソリューションの監視とトラブルシューティング
...
このラボでは、以下のタスクを完了します:
- Task 1: 開発環境を準備します。
- Task 2: Azure Cosmos DB SQL API アカウントを作成します。
...
## 演習 1: Azure Cosmos DB SQL API アカウントのキーを Azure Key Vault に保存する
...
Azure Cosmos DB は、複数の API をサポートするクラウドベースの NoSQL データベース サービスです。Azure Cosmos DB アカウントを初めてプロビジョニングする際には、サポートする API を選択します（たとえば、**Mongo API** や **SQL API**）。Azure Cosmos DB SQL API アカウントのプロビジョニングが完了したら、エンドポイントとキーを取得できます。これらのエンドポイントとキーを使用して、Azure Cosmos DB SQL API アカウントにプログラムから接続します。.NET 用 Azure SDK やその他の SDK の接続文字列にエンドポイントとキーを使用します。
